defmodule IslandsEngine.Player do
  ...
  @spec guess_coordinate(atom | pid | {atom, any} | {:via, atom, any}, atom) :: :hit | :miss
  ...
  defp convert_coordinates(board, coordinates) when is_list(coordinates) do
    Enum.map(coordinates, fn coord -> convert_coordinates(board, coord) end)
  end

  defp convert_coordinates(board, coordinate) when is_atom(coordinate) do
    Board.get_coordinate(board, coordinate)
  end

  defp convert_coordinates(_board, coordinate) when is_pid(coordinate) do
    coordinate
  end
